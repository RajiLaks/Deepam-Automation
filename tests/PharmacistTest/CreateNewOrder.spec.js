...
test.describe.serial('Pharmacist', () => {
    ...
    test.skip('TC002 - Create a New Order with Valid Credential', async () => {
        ...
        const { CustomerName, SearchMed, BatchNo, Quantity, Discount, Ref_Type, Reference, PaymentMode, Received } = data[1]
        const Create = new CreateNewOrder(page);
        const His = new History(page);
        await Create.CreateNewOrder();
        await Create.NewOrder(CustomerName, SearchMed, BatchNo, data[2].Quantity, Discount, data[0].Ref_Type, Reference, PaymentMode, Received);
        await Create.ConfirmYes();
        await His.Back();

    });
    test.skip('TC003 - Check Application displays Alert Message for Invalid Purchase of Medicine', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        console.log("exceldata:", data);

        const { CustomerName, Ref_Type, RefBy, Received } = data[2]
        const Create = new CreateNewOrder(page);
        await Create.CreateNewOrder();
    ...
    test.skip('TC004 - Create a New Order with 50% Discount', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        const { CustomerName, SearchMed, BatchNo, Quantity, Discount, Ref_Type, RefBy, PaymentMode, Received } = data[2]
        const Create = new CreateNewOrder(page);
        const His = new History(page);
         await Create.CreateNewOrder();
        //await page.reload()
        await Create.NewOrder(CustomerName, SearchMed, BatchNo, Quantity, data[4].Discount, Ref_Type, RefBy, PaymentMode, Received);
        await Create.ConfirmYes()
        await His.Back();


    });
    test('TC005 - Create a New Order with 50% Discount and without entering Ref_By and Paymentmode', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        const { CustomerName, SearchMed, BatchNo, Quantity, Discount, Amount, Ref_Type, RefBy, PaymentMode, Received } = data[2]
        const Create = new CreateNewOrder(page);
        await Create.CreateNewOrder();
        //await Create.NewOrder(CustomerName, SearchMed, BatchNo, Quantity, data[4].Discount, Ref_Type, data[0].RefBy, data[0].PaymentMode, Received);            
        await Create.CustomerName(CustomerName);
        await Create.Search_Madicine(SearchMed);
        await Create.Batch(BatchNo);
        await Create.Quantity(Quantity);
        await Create.Add_button();
        await Create.Discount(data[4].Discount);
        await Create.Reference(Ref_Type,'');
        await Create.Received(Received);
        await Create.Pay_Button();
       // await page.reload()
        
    });
    test('TC006 - Create and Edit the Medicine and pay', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        // console.log("Excel data:", data);
        // console.log("Excel data:", data[2]);

        const { CustomerName, SearchMed, BatchNo, Quantity, Received } = data[2]
        const Create = new CreateNewOrder(page);
        const His = new History(page);
        // await Create.CreateNewOrder();
        await page.reload()
        await Create.NewOrder(CustomerName, SearchMed, BatchNo, Quantity, "", "", "", data[1].PaymentMode, Received);
    ...
    test('TC007 - Create New Order with Multiple Medicine and pay', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        //     console.log("Excel data:", data);
        const Create = new CreateNewOrder(page);
        const His = new History(page);
    ...
    test('TC008 - Add Multiple Medicine then delete a medicine and pay', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        // console.log("Excel data:", data);
        const Create = new CreateNewOrder(page);
        const His = new History(page);
    ...
    test('TC009 - Goto Create Order page and View Ordered_History', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        console.log("Excel data:", data);
        const Create = new CreateNewOrder(page);
        const His = new History(page);
    ...
    test('TC010 - Goto Reorder page and order the same medicine', async () => {
        const data = await excelreader.readExcel('Utils/Deepam_Dataset.xlsx', 'Pharmacist');
        console.log("Excel data:", data);
        const Create = new CreateNewOrder(page);
        const His = new History(page);
